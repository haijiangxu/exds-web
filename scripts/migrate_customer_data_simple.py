...
def migrate_customer_data():
    ...
    try:
        ...
        print(f"[OK] 读取用户档案数据: {len(profiles_dict)} 个档案")
    except Exception as e:
        print(f"[ERROR] 读取用户档案数据失败: {e}")
        return
    ...
    try:
        ...
        print(f"[OK] 读取电表数据: {len(meters_dict)} 个电表")
    except Exception as e:
        print(f"[ERROR] 读取电表数据失败: {e}")
        return
    ...
    try:
        ...
        print(f"[OK] 读取计量点数据: {len(measure_points_dict)} 个计量点")
    except Exception as e:
        print(f"[ERROR] 读取计量点数据失败: {e}")
        return
    ...
    print(f"[OK] 构建客户数据: {len(customers)} 个客户")

    # 6. 检查是否已存在customers集合
    existing_count = DATABASE['customers'].count_documents({})
    if existing_count > 0:
        print(f"[WARNING] customers集合已存在 {existing_count} 条记录")
        print("将清空现有数据并重新迁移...")
        DATABASE['customers'].delete_many({})
        print("[OK] 已清空customers集合")

    # 7. 插入到customers集合
    try:
        if customers:
            result = DATABASE['customers'].insert_many(customers)
            print(f"[OK] 成功迁移 {len(result.inserted_ids)} 个客户数据")
...
def verify_migration():
    ...
    try:
        ...
        print(f"[OK] customers集合中的客户数量: {customers_count}")
