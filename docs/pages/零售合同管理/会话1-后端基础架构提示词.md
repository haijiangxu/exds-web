# 零售合同管理模块 - 会话1：后端基础架构 + API

## 📋 会话目标

完成零售合同管理模块的后端数据模型和基础 CRUD API，并进行完整的验证测试。

**预计工时**：2-3小时

---

## ⚠️ 开始前必读

在编写任何代码之前，请确认：

1. ✅ 已阅读 `docs/pages/零售合同管理/零售合同管理模块设计方案.md`
2. ✅ 已阅读 `docs/pages/零售合同管理/零售合同管理模块开发规划.md`
3. ✅ 已阅读 `CLAUDE.md` 中的后端开发规范
4. ✅ 测试账号信息：用户名 `admin`，密码 `!234qwer`

---

## 🔍 第一步：强制代码分析（必须完成）

**重要提示**：禁止直接开始编码！必须先分析参考模块的实际代码实现。

### 任务1：分析零售套餐模块的后端实现

**请依次读取并分析以下文件**：

```
1. webapp/models/retail_package.py
2. webapp/api/v1_retail_packages.py
3. webapp/services/retail_package_service.py (如果存在)
```

**分析重点并回答以下问题**：

**关于数据模型**：
- [ ] `BaseMongoModel` 是如何定义的？它继承自什么？
- [ ] `PyObjectId` 类型是如何处理的？具体实现代码是什么？
- [ ] 审计字段（created_at, created_by, updated_at, updated_by）的定义方式和默认值是什么？
- [ ] 如果有嵌套模型，它们是如何定义的？
- [ ] Pydantic 的 `model_config` 配置了哪些选项？

**关于 API 路由**：
- [ ] API 路由的前缀是什么？（例如 `/api/v1/retail-packages`）
- [ ] `APIRouter` 是如何初始化的？tags 参数是什么？
- [ ] JWT 认证是如何集成的？`Depends(get_current_active_user)` 的具体用法？
- [ ] 创建操作使用什么响应状态码？（201 Created？）
- [ ] 删除操作使用什么响应状态码？（204 No Content？）
- [ ] `HTTPException` 是如何使用的？错误消息的格式是什么？

**关于数据库操作**：
- [ ] `DATABASE` 是如何导入的？（从哪个模块？）
- [ ] 集合是如何访问的？（`DATABASE["collection_name"]` 还是其他方式？）
- [ ] 查询操作的标准模式是什么？
- [ ] 分页查询是如何实现的？
- [ ] 插入、更新、删除操作的具体代码模式？

**关于业务逻辑**：
- [ ] 状态机是如何实现的？有独立的函数吗？
- [ ] 虚拟字段（如状态）是如何计算的？
- [ ] 数据校验在哪里进行？（Pydantic 模型？API 层？服务层？）

**请在终端输出你的分析结论**，例如：
```
✓ BaseMongoModel 继承自 BaseModel，包含 PyObjectId 类型的 id 字段，使用 alias="_id"
✓ API 路由前缀为 /api/v1/retail-packages，使用 APIRouter(prefix=..., tags=["Retail Packages"])
✓ 所有受保护路由使用 current_user: User = Depends(get_current_active_user) 参数
✓ 数据库操作通过 from webapp.tools.mongo import DATABASE，然后 DATABASE["retail_packages"]
✓ 创建操作返回 201，使用 status_code=201 装饰器参数
✓ 删除操作返回 204，使用 status_code=204 装饰器参数
...
```

### 任务2：分析客户档案模块的后端实现

**请读取并分析**：

```
1. webapp/models/customer.py
2. webapp/api/v1_customers.py
```

**分析重点**：
- [ ] 嵌套数据模型的定义方式（如果有）
- [ ] 嵌套资源的 API 设计（如户号、计量点管理）
- [ ] 数据关联的处理方式

---

## 💻 第二步：开发任务

### 2.1 创建数据模型文件

**文件位置**：`webapp/models/contract.py`

**要求**：
- 严格参考零售套餐模块的模型定义方式
- 必须包含 `BaseMongoModel`、`PyObjectId` 的定义（如果项目中没有统一的基类）
- 定义以下模型：
  - `ContractCreate`：创建合同的输入模型
  - `ContractUpdate`：更新合同的输入模型（可选）
  - `Contract`：完整的合同数据模型，继承 `BaseMongoModel` 和 `ContractCreate`

**数据字段**（参考设计方案第3节）：
```python
class ContractCreate(BaseModel):
    package_name: str = Field(..., min_length=1)
    package_id: str = Field(..., min_length=1)
    customer_name: str = Field(..., min_length=1)
    customer_id: str = Field(..., min_length=1)
    purchasing_electricity_quantity: float = Field(..., gt=0)
    purchase_start_month: datetime  # 存储为每月1号
    purchase_end_month: datetime    # 存储为每月1号

class Contract(BaseMongoModel, ContractCreate):
    created_by: Optional[str] = None
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_by: Optional[str] = None
    updated_at: datetime = Field(default_factory=datetime.utcnow)

    # 虚拟字段（由后端计算，不存储在数据库）
    status: Optional[str] = None  # 'pending' | 'active' | 'expired'
```

### 2.2 创建状态计算函数

**在 `webapp/models/contract.py` 或独立的工具文件中**：

```python
def calculate_contract_status(purchase_start_month: datetime, purchase_end_month: datetime) -> str:
    """
    计算合同状态（虚拟字段）

    规则：
    - 待生效 (pending): 当前月份 < 购电开始月份
    - 生效 (active): 当前月份 >= 购电开始月份 且 <= 购电结束月份
    - 已过期 (expired): 当前月份 > 购电结束月份
    """
    from datetime import datetime

    now = datetime.utcnow()
    current_month = datetime(now.year, now.month, 1)
    start_month = datetime(purchase_start_month.year, purchase_start_month.month, 1)
    end_month = datetime(purchase_end_month.year, purchase_end_month.month, 1)

    if current_month < start_month:
        return "pending"
    elif current_month > end_month:
        return "expired"
    else:
        return "active"
```

### 2.3 创建 API 路由文件

**文件位置**：`webapp/api/v1_retail_contracts.py`

**要求**：
- 严格参考 `v1_retail_packages.py` 的路由结构
- 实现以下端点：

```python
from fastapi import APIRouter, Depends, HTTPException, Query
from typing import Optional, List
from webapp.models.contract import Contract, ContractCreate, calculate_contract_status
from webapp.tools.mongo import DATABASE
from webapp.api.dependencies import get_current_active_user  # 根据实际项目调整导入路径

router = APIRouter(prefix="/api/v1/retail-contracts", tags=["Retail Contracts"])

# 1. 获取合同列表（支持筛选、分页）
@router.get("", response_model=dict)
async def get_contracts(
    package_name: Optional[str] = None,
    customer_name: Optional[str] = None,
    status: Optional[str] = None,
    page: int = Query(1, ge=1),
    page_size: int = Query(10, ge=1, le=100),
    current_user: User = Depends(get_current_active_user)
):
    """
    获取合同列表

    支持筛选：
    - package_name: 套餐名称（模糊搜索）
    - customer_name: 客户名称（模糊搜索）
    - status: 合同状态（pending/active/expired）

    支持分页：
    - page: 页码（从1开始）
    - page_size: 每页数量
    """
    # TODO: 实现查询逻辑
    # 1. 构建查询条件
    # 2. 执行数据库查询
    # 3. 为每条记录计算虚拟状态字段
    # 4. 如果有 status 筛选，过滤结果
    # 5. 返回分页结果
    pass

# 2. 创建合同
@router.post("", status_code=201, response_model=Contract)
async def create_contract(
    contract: ContractCreate,
    current_user: User = Depends(get_current_active_user)
):
    """创建合同"""
    # TODO: 实现创建逻辑
    # 1. 验证套餐存在且状态为 active
    # 2. 验证客户存在且状态为 active
    # 3. 验证购电结束月份 >= 购电开始月份
    # 4. 添加审计字段
    # 5. 插入数据库
    # 6. 计算并添加虚拟状态字段
    # 7. 返回创建的合同
    pass

# 3. 获取合同详情
@router.get("/{contract_id}", response_model=Contract)
async def get_contract(
    contract_id: str,
    current_user: User = Depends(get_current_active_user)
):
    """获取合同详情"""
    # TODO: 实现获取逻辑
    # 1. 查询数据库
    # 2. 如果不存在，返回 404
    # 3. 计算并添加虚拟状态字段
    # 4. 返回合同
    pass

# 4. 更新合同
@router.put("/{contract_id}", response_model=Contract)
async def update_contract(
    contract_id: str,
    contract: ContractCreate,
    current_user: User = Depends(get_current_active_user)
):
    """更新合同（仅待生效状态）"""
    # TODO: 实现更新逻辑
    # 1. 查询现有合同
    # 2. 计算当前状态
    # 3. 如果状态不是 pending，返回 400 错误
    # 4. 验证套餐和客户存在性
    # 5. 更新审计字段
    # 6. 执行更新
    # 7. 返回更新后的合同
    pass

# 5. 删除合同
@router.delete("/{contract_id}", status_code=204)
async def delete_contract(
    contract_id: str,
    current_user: User = Depends(get_current_active_user)
):
    """删除合同（仅待生效状态）"""
    # TODO: 实现删除逻辑
    # 1. 查询现有合同
    # 2. 计算当前状态
    # 3. 如果状态不是 pending，返回 400 错误
    # 4. 执行删除
    # 5. 返回 204 No Content
    pass
```

### 2.4 更新主路由

**文件位置**：`webapp/api/v1.py`

**修改**：
```python
from webapp.api import v1_retail_contracts

# 在路由注册部分添加
router.include_router(v1_retail_contracts.router)
```

### 2.5 创建数据库索引

**文件位置**：`scripts/create_contract_indexes.py`

```python
from webapp.tools.mongo import DATABASE

def create_indexes():
    """创建合同集合的索引"""
    collection = DATABASE["retail_contracts"]

    # 单字段索引
    collection.create_index("package_name")
    collection.create_index("customer_name")
    collection.create_index("purchase_start_month")
    collection.create_index("purchase_end_month")

    # 复合索引（优化筛选查询）
    collection.create_index([
        ("package_name", 1),
        ("purchase_start_month", -1)
    ])

    print("✓ 合同集合索引创建完成")

if __name__ == "__main__":
    create_indexes()
```

---

## ✅ 第三步：验证测试

### 3.1 启动后端服务

使用自动化后端服务器管理（PowerShell）：

```powershell
# 启动后端（日志重定向）
Start-Process uvicorn -ArgumentList "webapp.main:app", "--reload", "--host", "0.0.0.0", "--port", "8005" -NoNewWindow -RedirectStandardOutput "tmp\uvicorn.out.log" -RedirectStandardError "tmp\uvicorn.err.log"

# 检查启动日志
cat tmp\uvicorn.err.log
```

### 3.2 获取 JWT Token

```powershell
# PowerShell
$response = Invoke-RestMethod -Uri "http://127.0.0.1:8005/token" -Method Post -Body @{username="admin";password="!234qwer"} -ContentType "application/x-www-form-urlencoded"
$TOKEN = $response.access_token
Write-Host "Token: $TOKEN"
```

### 3.3 运行完整的 API 测试

**测试脚本**（请依次执行）：

```powershell
# 1. 获取已生效的套餐列表
Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-packages?status=active" -Headers @{Authorization="Bearer $TOKEN"}

# 2. 获取正常状态的客户列表
Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/customers?status=active" -Headers @{Authorization="Bearer $TOKEN"}

# 3. 创建合同（使用上面获取的套餐ID和客户ID）
$contractData = @{
    package_name = "2024年商业尖峰平谷套餐"
    package_id = "<从步骤1获取的套餐ID>"
    customer_name = "丰城超胜塑业有限公司"
    customer_id = "<从步骤2获取的客户ID>"
    purchasing_electricity_quantity = 100000
    purchase_start_month = "2024-01-01T00:00:00Z"
    purchase_end_month = "2024-12-01T00:00:00Z"
} | ConvertTo-Json

$newContract = Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-contracts" -Method Post -Headers @{Authorization="Bearer $TOKEN"; "Content-Type"="application/json"} -Body $contractData
$contractId = $newContract._id
Write-Host "创建的合同ID: $contractId"

# 4. 获取合同列表
Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-contracts" -Headers @{Authorization="Bearer $TOKEN"}

# 5. 按套餐名称筛选
Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-contracts?package_name=商业" -Headers @{Authorization="Bearer $TOKEN"}

# 6. 按状态筛选（待生效）
Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-contracts?status=pending" -Headers @{Authorization="Bearer $TOKEN"}

# 7. 分页查询
Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-contracts?page=1&page_size=5" -Headers @{Authorization="Bearer $TOKEN"}

# 8. 获取合同详情
Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-contracts/$contractId" -Headers @{Authorization="Bearer $TOKEN"}

# 9. 更新合同（待生效状态应该成功）
$updateData = @{
    package_name = "2024年商业尖峰平谷套餐（已更新）"
    package_id = "<套餐ID>"
    customer_name = "丰城超胜塑业有限公司"
    customer_id = "<客户ID>"
    purchasing_electricity_quantity = 150000
    purchase_start_month = "2024-02-01T00:00:00Z"
    purchase_end_month = "2024-12-01T00:00:00Z"
} | ConvertTo-Json

Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-contracts/$contractId" -Method Put -Headers @{Authorization="Bearer $TOKEN"; "Content-Type"="application/json"} -Body $updateData

# 10. 删除合同（待生效状态应该成功）
Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-contracts/$contractId" -Method Delete -Headers @{Authorization="Bearer $TOKEN"}

# 11. 测试状态机：尝试更新已生效的合同（应返回400错误）
# 创建一个购电开始月份为过去的合同
$activeContractData = @{
    package_name = "2024年商业尖峰平谷套餐"
    package_id = "<套餐ID>"
    customer_name = "丰城超胜塑业有限公司"
    customer_id = "<客户ID>"
    purchasing_electricity_quantity = 100000
    purchase_start_month = "2023-01-01T00:00:00Z"
    purchase_end_month = "2025-12-01T00:00:00Z"
} | ConvertTo-Json

$activeContract = Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-contracts" -Method Post -Headers @{Authorization="Bearer $TOKEN"; "Content-Type"="application/json"} -Body $activeContractData
$activeContractId = $activeContract._id

# 尝试更新（应该失败）
try {
    Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-contracts/$activeContractId" -Method Put -Headers @{Authorization="Bearer $TOKEN"; "Content-Type"="application/json"} -Body $updateData
} catch {
    Write-Host "✓ 正确拒绝了更新已生效合同的请求: $($_.Exception.Message)"
}

# 尝试删除（应该失败）
try {
    Invoke-RestMethod -Uri "http://127.0.0.1:8005/api/v1/retail-contracts/$activeContractId" -Method Delete -Headers @{Authorization="Bearer $TOKEN"}
} catch {
    Write-Host "✓ 正确拒绝了删除已生效合同的请求: $($_.Exception.Message)"
}
```

### 3.4 访问 API 文档

浏览器访问：http://127.0.0.1:8005/docs

检查：
- [ ] `/api/v1/retail-contracts` 路由组是否出现
- [ ] 所有端点的请求/响应模型是否正确
- [ ] 能否通过交互式文档测试所有 API

### 3.5 数据库验证

使用 mongosh 或 MongoDB Compass：

```bash
# 连接数据库
mongosh "mongodb://localhost:27017/exds"

# 查看合同数据
db.retail_contracts.find().pretty()

# 验证索引
db.retail_contracts.getIndexes()

# 查询待生效的合同
db.retail_contracts.find({
  purchase_start_month: { $gt: new Date() }
})

# 查询已生效的合同
db.retail_contracts.find({
  purchase_start_month: { $lte: new Date() },
  purchase_end_month: { $gte: new Date() }
})
```

---

## 📝 验收标准

在完成开发后，请逐项检查：

**功能性**：
- [ ] 所有 5 个 CRUD API 正常工作
- [ ] JWT 认证集成正常（未认证请求返回 401）
- [ ] 虚拟状态字段计算正确（pending/active/expired）
- [ ] 状态机权限控制有效：
  - [ ] 待生效合同可以编辑
  - [ ] 待生效合同可以删除
  - [ ] 已生效合同不可编辑（返回 400）
  - [ ] 已生效合同不可删除（返回 400）
  - [ ] 已过期合同不可编辑（返回 400）
  - [ ] 已过期合同不可删除（返回 400）
- [ ] 分页功能正常
- [ ] 筛选功能正常（套餐名称、客户名称、状态）

**数据库**：
- [ ] 索引创建成功
- [ ] 数据结构符合设计方案
- [ ] 审计字段自动填充

**代码质量**：
- [ ] 所有代码添加了类型提示
- [ ] 遵循 PEP 8 规范
- [ ] 使用 `DATABASE` 全局实例操作数据库
- [ ] 使用 `HTTPException` 处理错误
- [ ] 命名规范：snake_case（变量/函数）、PascalCase（类）

**文档**：
- [ ] API 文档自动生成且准确
- [ ] 所有端点有清晰的 docstring
- [ ] 错误响应有明确的 detail 消息

---

## 🚨 常见问题和注意事项

1. **PyObjectId 类型错误**：
   - 确保 `PyObjectId` 的 `__get_pydantic_json_schema__` 方法正确实现
   - 确保在 `model_config` 中设置 `json_encoders={ObjectId: str}`

2. **JWT 认证导入错误**：
   - 检查 `get_current_active_user` 函数的导入路径
   - 可能在 `webapp/api/dependencies.py` 或 `webapp/auth.py` 中

3. **DATABASE 导入错误**：
   - 确认 `webapp/tools/mongo.py` 文件存在
   - 确认 `DATABASE` 是正确导出的全局变量

4. **日期处理问题**：
   - 确保月份统一存储为每月1号（`datetime(year, month, 1)`）
   - 使用 UTC 时间（`datetime.utcnow()`）

5. **状态计算逻辑**：
   - 确保状态计算函数在每次返回数据时都被调用
   - 状态字段不应存储在数据库中（它是虚拟字段）

---

## 📚 参考文档

- **设计方案**：`docs/pages/零售合同管理/零售合同管理模块设计方案.md`
- **开发规划**：`docs/pages/零售合同管理/零售合同管理模块开发规划.md`
- **项目规范**：`CLAUDE.md`
- **参考代码**：
  - `webapp/models/retail_package.py`
  - `webapp/api/v1_retail_packages.py`
  - `webapp/models/customer.py`
  - `webapp/api/v1_customers.py`

---

## ✅ 完成后的行动

在完成本会话的所有开发和测试后：

1. 更新开发规划文档中的进度表
2. 提交代码到 Git（如果项目使用版本控制）
3. 准备进入会话2（导入导出功能）

**会话2提示词**：`docs/pages/零售合同管理/会话2-导入导出功能提示词.md`
