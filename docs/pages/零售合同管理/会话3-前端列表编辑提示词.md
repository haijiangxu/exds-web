# 零售合同管理模块 - 会话3：前端列表和编辑功能

## 📋 会话目标

完成零售合同管理模块的前端列表页面和编辑对话框，实现与后端API的完整集成。

**预计工时**：2-3小时

**前置条件**：
- 会话1和会话2已完成
- 后端API正常工作（包括导入导出）
- 前端开发环境已配置

---

## ⚠️ 开始前必读

在编写任何代码之前，请确认：

1. ✅ 会话1和2已完成并通过所有验收标准
2. ✅ 后端服务正常运行（http://127.0.0.1:8005）
3. ✅ 前端开发服务器可以正常启动
4. ✅ 已阅读 `docs/前端页面设计规范.md`
5. ✅ 已阅读 `CLAUDE.md` 中的前端开发规范

---

## 🔍 第一步：强制代码分析（必须完成）

**重要提示**：禁止直接开始编码！必须先分析参考模块的前端实际代码实现。

### 任务1：分析零售套餐模块的列表页面

**请依次读取并分析以下文件**：

```
frontend/src/pages/RetailPackagePage.tsx
```

**分析重点并回答以下问题**：

**关于页面结构**：
- [ ] 外层容器使用什么组件？（`<Box>`？`<Container>`？）
- [ ] 外层容器的 `sx` 属性是什么？（`width: '100%'`？）
- [ ] 面包屑导航是如何实现的？使用什么组件？
- [ ] 面包屑的样式是什么？（`fontSize`、`fontWeight` 等）
- [ ] 是否有页面主标题？如果有，使用什么组件和样式？

**关于查询区域**：
- [ ] 查询卡片使用什么组件？（`Paper`？）
- [ ] `Paper` 的 `variant` 是什么？（`outlined`？`elevation`？）
- [ ] 内边距的响应式设置？（`p: { xs: 1, sm: 2 }`？）
- [ ] 查询字段的布局方式？（`Grid`？`Box`？）
- [ ] 按钮的样式配置？（`variant`、`color`）
- [ ] 筛选条件的状态管理方式？

**关于列表区域**：
- [ ] 列表卡片的样式配置？
- [ ] 工具栏的布局方式？
- [ ] 按钮的排列和样式？
- [ ] 表格组件的完整配置？
- [ ] 表格的响应式样式？（`fontSize`、`px`）
- [ ] 分页组件的位置和配置？

**关于操作按钮**：
- [ ] `renderTableActions` 函数是如何实现的？
- [ ] 如何根据状态控制按钮显示/禁用？
- [ ] 图标按钮使用什么组件？
- [ ] `Tooltip` 是如何使用的？

**关于对话框状态管理**：
- [ ] 使用哪些 `useState` 来控制对话框？
- [ ] 如何打开编辑对话框？
- [ ] 如何传递数据给编辑对话框？
- [ ] 对话框关闭后如何刷新列表？

**关于数据加载**：
- [ ] 数据加载的时机？（`useEffect`）
- [ ] Loading 状态如何显示？
- [ ] 错误处理如何实现？
- [ ] 分页逻辑如何处理？

**请在终端输出你的分析结论**，例如：
```
✓ 外层容器使用 <Box sx={{ width: '100%' }}>
✓ 面包屑使用 Breadcrumbs 组件，fontSize: '1.2rem', fontWeight: 600
✓ 查询卡片使用 Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 }, mb: 2 }}
✓ Grid 布局使用 size={{ xs: 12, md: 6 }} 语法
✓ 按钮使用 variant="contained" color="primary" 和 variant="outlined"
✓ renderTableActions 根据 package.status 动态设置按钮
...
```

### 任务2：分析编辑对话框实现

**请读取并分析**：

```
frontend/src/components/PackageEditorDialog.tsx
```

**分析重点**：
- [ ] `Dialog` 组件的完整配置（`open`、`onClose`、`maxWidth`、`fullScreen`、`disableEnforceFocus`）
- [ ] 移动端全屏的实现方式？
- [ ] `DialogContent` 的滚动容器配置？
- [ ] React Hook Form 的集成方式（`useForm`、`Controller`）
- [ ] 辅助渲染函数的组织方式（有哪些函数？）
- [ ] 表单字段的配置（`TextField`、`Select`、`Autocomplete`）
- [ ] Grid 布局的响应式配置？
- [ ] `DialogActions` 的按钮配置？
- [ ] 表单提交逻辑（`handleSubmit`）
- [ ] 错误处理和显示方式？

### 任务3：分析 API 客户端实现

**请读取并分析**：

```
frontend/src/api/retail-packages.ts
```

**分析重点**：
- [ ] 接口类型的定义方式？
- [ ] API 方法的命名规范？
- [ ] 请求和响应的类型标注？
- [ ] 错误处理模式？
- [ ] 导出方式（`export const` 还是 `export default`）？

---

## 💻 第二步：开发任务

### 2.1 创建 API 客户端

**文件位置**：`frontend/src/api/retail-contracts.ts`

**要求**：严格参考 `retail-packages.ts` 的实现模式

```typescript
import apiClient from './client';

// 类型定义
export interface Contract {
  _id: string;
  package_name: string;
  package_id: string;
  customer_name: string;
  customer_id: string;
  purchasing_electricity_quantity: number;
  purchase_start_month: string;
  purchase_end_month: string;
  status: 'pending' | 'active' | 'expired';
  created_at: string;
  updated_at: string;
  created_by?: string;
  updated_by?: string;
}

export interface ContractListResponse {
  items: Contract[];
  total: number;
  page: number;
  page_size: number;
}

export interface ContractFormData {
  package_name: string;
  package_id: string;
  customer_name: string;
  customer_id: string;
  purchasing_electricity_quantity: number;
  purchase_start_month: Date | null;
  purchase_end_month: Date | null;
}

// API 方法
export const contractApi = {
  getList: (params?: any) =>
    apiClient.get<ContractListResponse>('/api/v1/retail-contracts', { params }),

  getById: (id: string) =>
    apiClient.get<Contract>(`/api/v1/retail-contracts/${id}`),

  create: (data: Partial<Contract>) =>
    apiClient.post<Contract>('/api/v1/retail-contracts', data),

  update: (id: string, data: Partial<Contract>) =>
    apiClient.put<Contract>(`/api/v1/retail-contracts/${id}`, data),

  delete: (id: string) =>
    apiClient.delete(`/api/v1/retail-contracts/${id}`),

  import: (file: File) => {
    const formData = new FormData();
    formData.append('file', file);
    return apiClient.post('/api/v1/retail-contracts/import', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
  },

  export: (params?: any) =>
    apiClient.get('/api/v1/retail-contracts/export', {
      params,
      responseType: 'blob'
    })
};
```

### 2.2 创建合同列表页面

**文件位置**：`frontend/src/pages/RetailContractPage.tsx`

**强制要求**：
- 完全参考 `RetailPackagePage.tsx` 的结构
- 复用查询卡片和列表卡片的样式
- 实现 `renderTableActions` 函数

**核心代码结构**：

```typescript
import React, { useState, useEffect } from 'react';
import {
  Box, Paper, Typography, Breadcrumbs, TextField, Select, MenuItem,
  Button, Table, TableHead, TableBody, TableRow, TableCell,
  TableContainer, TablePagination, IconButton, Tooltip,
  CircularProgress, Alert, Grid
} from '@mui/material';
import VisibilityIcon from '@mui/icons-material/Visibility';
import EditIcon from '@mui/icons-material/Edit';
import DeleteIcon from '@mui/icons-material/Delete';
import { contractApi, Contract } from '../api/retail-contracts';
import ContractEditorDialog from '../components/ContractEditorDialog';
// ... 其他导入

export const RetailContractPage: React.FC = () => {
  // 1. 状态管理
  const [contracts, setContracts] = useState<Contract[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(10);
  const [total, setTotal] = useState(0);

  // 筛选条件
  const [filters, setFilters] = useState({
    package_name: '',
    customer_name: '',
    status: 'all',
  });

  // 对话框状态
  const [isEditorOpen, setIsEditorOpen] = useState(false);
  const [selectedContract, setSelectedContract] = useState<Contract | null>(null);
  const [editorMode, setEditorMode] = useState<'create' | 'edit'>('create');

  // 2. 数据加载
  const fetchContracts = async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await contractApi.getList({
        ...filters,
        status: filters.status === 'all' ? undefined : filters.status,
        page: page + 1,
        page_size: pageSize
      });
      setContracts(response.data.items);
      setTotal(response.data.total);
    } catch (err: any) {
      setError(err.response?.data?.detail || '加载合同列表失败');
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchContracts();
  }, [page, pageSize]);

  // 3. 操作处理
  const handleCreate = () => {
    setSelectedContract(null);
    setEditorMode('create');
    setIsEditorOpen(true);
  };

  const handleEdit = (contract: Contract) => {
    setSelectedContract(contract);
    setEditorMode('edit');
    setIsEditorOpen(true);
  };

  const handleDelete = async (contract: Contract) => {
    // TODO: 显示确认对话框
    // TODO: 调用删除 API
    // TODO: 刷新列表
  };

  const handleSearch = () => {
    setPage(0);
    fetchContracts();
  };

  const handleReset = () => {
    setFilters({
      package_name: '',
      customer_name: '',
      status: 'all',
    });
    setPage(0);
  };

  // 4. 渲染操作按钮（根据状态控制）
  const renderTableActions = (contract: Contract) => (
    <Box sx={{ display: 'flex', gap: 1 }}>
      <Tooltip title="查看">
        <IconButton size="small" onClick={() => handleView(contract)}>
          <VisibilityIcon fontSize="small" />
        </IconButton>
      </Tooltip>

      {contract.status === 'pending' && (
        <>
          <Tooltip title="编辑">
            <IconButton size="small" onClick={() => handleEdit(contract)}>
              <EditIcon fontSize="small" />
            </IconButton>
          </Tooltip>

          <Tooltip title="删除">
            <IconButton size="small" onClick={() => handleDelete(contract)}>
              <DeleteIcon fontSize="small" />
            </IconButton>
          </Tooltip>
        </>
      )}
    </Box>
  );

  // 5. UI 渲染
  return (
    <Box sx={{ width: '100%' }}>
      {/* 面包屑导航 */}
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 2 }}>
        <Typography sx={{ fontSize: '1.2rem', fontWeight: 600 }}>
          合同管理
        </Typography>
        <Typography sx={{ fontSize: '1.2rem', fontWeight: 600 }}>
          零售合同
        </Typography>
      </Breadcrumbs>

      {/* 查询区域 */}
      <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 }, mb: 2 }}>
        <Grid container spacing={{ xs: 1, sm: 2 }} alignItems="center">
          <Grid size={{ xs: 12, md: 3 }}>
            <TextField
              fullWidth
              size="small"
              label="套餐名称"
              value={filters.package_name}
              onChange={(e) => setFilters({ ...filters, package_name: e.target.value })}
            />
          </Grid>

          <Grid size={{ xs: 12, md: 3 }}>
            <TextField
              fullWidth
              size="small"
              label="客户名称"
              value={filters.customer_name}
              onChange={(e) => setFilters({ ...filters, customer_name: e.target.value })}
            />
          </Grid>

          <Grid size={{ xs: 12, md: 2 }}>
            <Select
              fullWidth
              size="small"
              value={filters.status}
              onChange={(e) => setFilters({ ...filters, status: e.target.value })}
            >
              <MenuItem value="all">所有状态</MenuItem>
              <MenuItem value="pending">待生效</MenuItem>
              <MenuItem value="active">生效</MenuItem>
              <MenuItem value="expired">已过期</MenuItem>
            </Select>
          </Grid>

          <Grid size={{ xs: 12, md: 4 }}>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Button
                variant="contained"
                color="primary"
                onClick={handleSearch}
              >
                查询
              </Button>
              <Button
                variant="outlined"
                onClick={handleReset}
              >
                重置
              </Button>
            </Box>
          </Grid>
        </Grid>
      </Paper>

      {/* 列表区域 */}
      <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 } }}>
        {/* 工具栏 */}
        <Box sx={{ mb: 2, display: 'flex', gap: 1 }}>
          <Button
            variant="contained"
            color="primary"
            onClick={handleCreate}
          >
            + 新增
          </Button>
          <Button
            variant="outlined"
            onClick={() => {/* TODO: 打开导入对话框 */}}
          >
            导入
          </Button>
          <Button
            variant="outlined"
            onClick={() => {/* TODO: 导出 */}}
          >
            导出
          </Button>
        </Box>

        {/* 数据表格 */}
        {loading ? (
          <Box sx={{ display: 'flex', justifyContent: 'center', p: 4 }}>
            <CircularProgress />
          </Box>
        ) : error ? (
          <Alert severity="error">{error}</Alert>
        ) : (
          <>
            <TableContainer sx={{ overflowX: 'auto' }}>
              <Table sx={{
                '& .MuiTableCell-root': {
                  fontSize: { xs: '0.75rem', sm: '0.875rem' },
                  px: { xs: 0.5, sm: 2 },
                }
              }}>
                <TableHead>
                  <TableRow>
                    <TableCell>合同编号</TableCell>
                    <TableCell>套餐名称</TableCell>
                    <TableCell>客户名称</TableCell>
                    <TableCell>购买电量</TableCell>
                    <TableCell>购电开始月份</TableCell>
                    <TableCell>购电结束月份</TableCell>
                    <TableCell>状态</TableCell>
                    <TableCell>操作</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {contracts.map((contract) => (
                    <TableRow key={contract._id}>
                      <TableCell>{contract._id}</TableCell>
                      <TableCell>{contract.package_name}</TableCell>
                      <TableCell>{contract.customer_name}</TableCell>
                      <TableCell>{contract.purchasing_electricity_quantity}</TableCell>
                      <TableCell>{contract.purchase_start_month}</TableCell>
                      <TableCell>{contract.purchase_end_month}</TableCell>
                      <TableCell>{contract.status}</TableCell>
                      <TableCell>{renderTableActions(contract)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            </TableContainer>

            {/* 分页 */}
            <TablePagination
              component="div"
              count={total}
              page={page}
              onPageChange={(e, newPage) => setPage(newPage)}
              rowsPerPage={pageSize}
              onRowsPerPageChange={(e) => {
                setPageSize(parseInt(e.target.value, 10));
                setPage(0);
              }}
              labelRowsPerPage="每页行数"
            />
          </>
        )}
      </Paper>

      {/* 编辑对话框 */}
      <ContractEditorDialog
        open={isEditorOpen}
        onClose={() => setIsEditorOpen(false)}
        contract={selectedContract}
        mode={editorMode}
        onSuccess={() => {
          setIsEditorOpen(false);
          fetchContracts();
        }}
      />
    </Box>
  );
};

export default RetailContractPage;
```

### 2.3 创建合同编辑对话框

**文件位置**：`frontend/src/components/ContractEditorDialog.tsx`

**强制要求**：
- 完全参考 `PackageEditorDialog.tsx` 的结构
- 使用 React Hook Form
- 实现辅助渲染函数

**核心代码结构**：

```typescript
import React, { useState, useEffect } from 'react';
import {
  Dialog, DialogTitle, DialogContent, DialogActions,
  Button, TextField, Paper, Typography, Grid, Autocomplete
} from '@mui/material';
import { useForm, Controller } from 'react-hook-form';
import { DatePicker, LocalizationProvider } from '@mui/x-date-pickers';
import { AdapterDateFns } from '@mui/x-date-pickers/AdapterDateFns';
import { zhCN } from 'date-fns/locale';
import { contractApi, ContractFormData } from '../api/retail-contracts';
import apiClient from '../api/client';

interface Props {
  open: boolean;
  onClose: () => void;
  contract: Contract | null;
  mode: 'create' | 'edit';
  onSuccess: () => void;
}

export const ContractEditorDialog: React.FC<Props> = ({
  open,
  onClose,
  contract,
  mode,
  onSuccess
}) => {
  // 1. 表单管理
  const { control, handleSubmit, watch, setValue, reset, formState: { errors } } = useForm<ContractFormData>({
    defaultValues: {
      package_name: '',
      package_id: '',
      customer_name: '',
      customer_id: '',
      purchasing_electricity_quantity: 0,
      purchase_start_month: null,
      purchase_end_month: null
    }
  });

  // 2. 关联数据加载
  const [packageOptions, setPackageOptions] = useState([]);
  const [customerOptions, setCustomerOptions] = useState([]);
  const [selectedPackage, setSelectedPackage] = useState<any>(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (open) {
      // 加载已生效的套餐列表
      apiClient.get('/api/v1/retail-packages?status=active')
        .then(res => setPackageOptions(res.data.items || []))
        .catch(err => console.error('加载套餐列表失败', err));

      // 加载正常状态的客户列表
      apiClient.get('/api/v1/customers?status=active')
        .then(res => setCustomerOptions(res.data.items || []))
        .catch(err => console.error('加载客户列表失败', err));

      // 如果是编辑模式，填充表单
      if (mode === 'edit' && contract) {
        reset({
          package_name: contract.package_name,
          package_id: contract.package_id,
          customer_name: contract.customer_name,
          customer_id: contract.customer_id,
          purchasing_electricity_quantity: contract.purchasing_electricity_quantity,
          purchase_start_month: new Date(contract.purchase_start_month),
          purchase_end_month: new Date(contract.purchase_end_month)
        });
      }
    }
  }, [open, mode, contract]);

  // 3. 套餐选择后加载详情
  const handlePackageSelect = async (packageId: string) => {
    try {
      const response = await apiClient.get(`/api/v1/retail-packages/${packageId}`);
      setSelectedPackage(response.data);
    } catch (error) {
      console.error('加载套餐详情失败', error);
    }
  };

  // 4. 表单提交
  const onSubmit = async (data: ContractFormData) => {
    setLoading(true);
    try {
      const submitData = {
        ...data,
        purchase_start_month: data.purchase_start_month?.toISOString(),
        purchase_end_month: data.purchase_end_month?.toISOString()
      };

      if (mode === 'create') {
        await contractApi.create(submitData);
      } else {
        await contractApi.update(contract!._id, submitData);
      }

      onSuccess();
    } catch (error: any) {
      console.error('保存失败', error);
      // TODO: 显示错误提示
    } finally {
      setLoading(false);
    }
  };

  // 5. 辅助渲染函数 - 基本信息
  const renderBasicInfoSection = () => (
    <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 }, mt: 2 }}>
      <Typography variant="h6" gutterBottom>合同基本信息</Typography>

      <Grid container spacing={{ xs: 1, sm: 2 }}>
        {/* 套餐名称 */}
        <Grid size={{ xs: 12, md: 6 }}>
          <Controller
            name="package_name"
            control={control}
            rules={{ required: '请选择套餐' }}
            render={({ field }) => (
              <Autocomplete
                {...field}
                options={packageOptions}
                getOptionLabel={(option: any) => option.package_name || ''}
                onChange={(event, value) => {
                  field.onChange(value?.package_name || '');
                  setValue('package_id', value?._id || '');
                  if (value) handlePackageSelect(value._id);
                }}
                renderInput={(params) => (
                  <TextField
                    {...params}
                    label="套餐名称"
                    required
                    error={!!errors.package_name}
                    helperText={errors.package_name?.message}
                  />
                )}
              />
            )}
          />
        </Grid>

        {/* 客户名称 */}
        <Grid size={{ xs: 12, md: 6 }}>
          <Controller
            name="customer_name"
            control={control}
            rules={{ required: '请选择客户' }}
            render={({ field }) => (
              <Autocomplete
                {...field}
                options={customerOptions}
                getOptionLabel={(option: any) => option.user_name || ''}
                onChange={(event, value) => {
                  field.onChange(value?.user_name || '');
                  setValue('customer_id', value?._id || '');
                }}
                renderInput={(params) => (
                  <TextField
                    {...params}
                    label="客户名称"
                    required
                    error={!!errors.customer_name}
                    helperText={errors.customer_name?.message}
                  />
                )}
              />
            )}
          />
        </Grid>

        {/* 购买电量 */}
        <Grid size={{ xs: 12, md: 6 }}>
          <Controller
            name="purchasing_electricity_quantity"
            control={control}
            rules={{
              required: '请输入购买电量',
              min: { value: 0.01, message: '购买电量必须大于0' }
            }}
            render={({ field }) => (
              <TextField
                {...field}
                fullWidth
                label="购买电量 (kWh)"
                type="number"
                required
                inputProps={{ min: 0, step: 0.01 }}
                error={!!errors.purchasing_electricity_quantity}
                helperText={errors.purchasing_electricity_quantity?.message}
              />
            )}
          />
        </Grid>

        {/* 购电开始月份 */}
        <Grid size={{ xs: 12, md: 6 }}>
          <Controller
            name="purchase_start_month"
            control={control}
            rules={{ required: '请选择购电开始月份' }}
            render={({ field }) => (
              <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={zhCN}>
                <DatePicker
                  {...field}
                  label="购电开始月份"
                  views={['year', 'month']}
                  slotProps={{
                    textField: {
                      fullWidth: true,
                      required: true,
                      error: !!errors.purchase_start_month,
                      helperText: errors.purchase_start_month?.message
                    }
                  }}
                />
              </LocalizationProvider>
            )}
          />
        </Grid>

        {/* 购电结束月份 */}
        <Grid size={{ xs: 12, md: 6 }}>
          <Controller
            name="purchase_end_month"
            control={control}
            rules={{
              required: '请选择购电结束月份',
              validate: (value) => {
                const startMonth = watch('purchase_start_month');
                if (startMonth && value && value < startMonth) {
                  return '购电结束月份不能早于开始月份';
                }
                return true;
              }
            }}
            render={({ field }) => (
              <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={zhCN}>
                <DatePicker
                  {...field}
                  label="购电结束月份"
                  views={['year', 'month']}
                  slotProps={{
                    textField: {
                      fullWidth: true,
                      required: true,
                      error: !!errors.purchase_end_month,
                      helperText: errors.purchase_end_month?.message
                    }
                  }}
                />
              </LocalizationProvider>
            )}
          />
        </Grid>
      </Grid>
    </Paper>
  );

  // 6. 辅助渲染函数 - 套餐内容（只读）
  const renderPackageContentSection = () => (
    <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 }, mt: 2 }}>
      <Typography variant="h6" gutterBottom>关联套餐内容</Typography>

      {selectedPackage ? (
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, sm: 6 }}>
            <Typography variant="body2" color="text.secondary">套餐类型</Typography>
            <Typography variant="body1">
              {selectedPackage.package_type === 'time_based' ? '分时段' : '不分时段'}
            </Typography>
          </Grid>

          <Grid size={{ xs: 12, sm: 6 }}>
            <Typography variant="body2" color="text.secondary">定价模式</Typography>
            <Typography variant="body1">
              {selectedPackage.pricing_mode === 'fixed_linked' ? '固定+联动' : '价差分成'}
            </Typography>
          </Grid>

          <Grid size={{ xs: 12, sm: 6 }}>
            <Typography variant="body2" color="text.secondary">是否绿电</Typography>
            <Typography variant="body1">
              {selectedPackage.additional_terms?.is_green_energy ? '是' : '否'}
            </Typography>
          </Grid>

          {/* TODO: 根据实际套餐数据结构展示更多字段 */}
        </Grid>
      ) : (
        <Typography color="text.secondary">请先选择套餐</Typography>
      )}
    </Paper>
  );

  // 7. 主渲染
  return (
    <Dialog
      open={open}
      onClose={onClose}
      maxWidth="md"
      fullWidth
      fullScreen={window.innerWidth < 600}  // 移动端全屏
      disableEnforceFocus
    >
      <DialogTitle>
        {mode === 'create' ? '新增合同' : '编辑合同'}
      </DialogTitle>

      <DialogContent>
        <form id="contract-form" onSubmit={handleSubmit(onSubmit)}>
          {renderBasicInfoSection()}
          {renderPackageContentSection()}
        </form>
      </DialogContent>

      <DialogActions>
        <Button onClick={onClose}>取消</Button>
        <Button
          type="submit"
          form="contract-form"
          variant="contained"
          color="primary"
          disabled={loading}
        >
          保存
        </Button>
      </DialogActions>
    </Dialog>
  );
};

export default ContractEditorDialog;
```

### 2.4 更新路由配置

**文件位置**：`frontend/src/App.tsx`

**添加路由**：

```typescript
import RetailContractPage from './pages/RetailContractPage';
import ProtectedRoute from './components/ProtectedRoute';

// 在路由配置中添加
<Route path="/retail-contracts" element={
  <ProtectedRoute>
    <RetailContractPage />
  </ProtectedRoute>
} />
```

---

## ✅ 第三步：验证测试

### 3.1 启动前端服务

```powershell
# 后台启动（日志重定向）
Start-Process npm -ArgumentList "start", "--prefix", "frontend" -NoNewWindow -RedirectStandardOutput "tmp\frontend_dev.log" -RedirectStandardError "tmp\frontend_dev.log"

# 监控编译错误
cat tmp\frontend_dev.log

# 或运行构建检查
npm run build --prefix frontend
```

### 3.2 使用 Chrome DevTools 调试

**重要**：使用 chrome-devtools MCP 工具进行系统化测试

```typescript
// 1. 打开应用并登录
await mcp__chrome-devtools__new_page({ url: 'http://localhost:3000' })
await mcp__chrome-devtools__take_snapshot()

// 2. 登录（假设已有登录表单）
await mcp__chrome-devtools__fill({ uid: 'username-field-uid', value: 'admin' })
await mcp__chrome-devtools__fill({ uid: 'password-field-uid', value: '!234qwer' })
await mcp__chrome-devtools__click({ uid: 'login-button-uid' })

// 3. 导航到合同管理页面
await mcp__chrome-devtools__click({ uid: 'contract-menu-uid' })
await mcp__chrome-devtools__take_snapshot()

// 4. 测试查询功能
await mcp__chrome-devtools__fill({ uid: 'package-name-field-uid', value: '商业' })
await mcp__chrome-devtools__click({ uid: 'search-button-uid' })
await mcp__chrome-devtools__take_snapshot()

// 5. 测试新增功能
await mcp__chrome-devtools__click({ uid: 'create-button-uid' })
await mcp__chrome-devtools__take_snapshot()

// 6. 检查控制台错误
await mcp__chrome-devtools__list_console_messages({ types: ['error', 'warn'] })
```

### 3.3 功能测试清单

**列表页面测试**：
- [ ] 页面加载正常，面包屑导航显示"合同管理 > 零售合同"
- [ ] 合同列表数据正常显示
- [ ] 筛选功能：
  - [ ] 按套餐名称筛选有效
  - [ ] 按客户名称筛选有效
  - [ ] 按状态筛选有效（所有/待生效/生效/已过期）
- [ ] 分页功能：
  - [ ] 切换页码正常
  - [ ] 调整每页数量正常
  - [ ] 总数显示正确
- [ ] 刷新和重置按钮功能正常
- [ ] 响应式设计：
  - [ ] 桌面端布局正确
  - [ ] 移动端布局正确（字段垂直堆叠）

**操作按钮测试**：
- [ ] 待生效状态的合同显示"查看"、"编辑"、"删除"按钮
- [ ] 已生效状态的合同仅显示"查看"按钮
- [ ] 已过期状态的合同仅显示"查看"按钮
- [ ] 按钮 Tooltip 显示正确

**编辑对话框测试（新增）**：
- [ ] 点击"新增合同"按钮，对话框打开
- [ ] 对话框标题显示"新增合同"
- [ ] 套餐选择下拉列表：
  - [ ] 加载正常（仅显示已生效套餐）
  - [ ] 支持搜索/过滤
  - [ ] 可以正常选择
- [ ] 客户选择下拉列表：
  - [ ] 加载正常（仅显示正常状态客户）
  - [ ] 支持搜索/过滤
  - [ ] 可以正常选择
- [ ] 选择套餐后：
  - [ ] 套餐内容卡片自动展示详情
  - [ ] 套餐类型显示正确
  - [ ] 定价模式显示正确
  - [ ] 是否绿电显示正确
- [ ] 月份选择器：
  - [ ] 仅显示年月选择
  - [ ] 可以正常选择
  - [ ] 格式正确
- [ ] 表单验证：
  - [ ] 必填项提示
  - [ ] 购买电量>0验证
  - [ ] 结束月份>=开始月份验证
- [ ] 保存功能：
  - [ ] 保存成功后显示成功提示
  - [ ] 对话框关闭
  - [ ] 列表自动刷新
  - [ ] 新合同出现在列表中

**编辑对话框测试（编辑）**：
- [ ] 点击待生效合同的"编辑"按钮，对话框打开
- [ ] 对话框标题显示"编辑合同"
- [ ] 表单字段自动填充现有数据
- [ ] 可以修改数据
- [ ] 保存成功后列表更新

**移动端测试**：
- [ ] 对话框自动切换为全屏模式
- [ ] 表单字段单列垂直布局
- [ ] 所有操作正常

---

## 📝 验收标准

在完成开发后，请逐项检查：

**功能性**：
- [ ] 所有列表和编辑功能正常工作
- [ ] API 调用正常，数据联动正确
- [ ] 操作按钮根据状态正确显示/隐藏
- [ ] 表单验证完整且准确
- [ ] 关联数据（套餐、客户）加载正常
- [ ] 套餐内容自动展示功能正常

**UI/UX**：
- [ ] UI 与零售套餐模块高度一致
- [ ] 响应式设计在所有设备上正常
- [ ] 移动端全屏模式正常
- [ ] Loading 状态提示清晰
- [ ] 错误提示友好且明确

**代码质量**：
- [ ] 使用 TypeScript，类型定义完整
- [ ] 组件命名 PascalCase
- [ ] 变量函数使用 camelCase
- [ ] Grid 组件使用 `size` 属性（MUI v7）
- [ ] 遵循项目前端页面设计规范

**测试**：
- [ ] 无 JavaScript 错误或警告
- [ ] 浏览器控制台无报错
- [ ] 网络请求正常（查看 Network 面板）

---

## 🚨 常见问题和注意事项

1. **Grid 组件语法错误**：
   - ❌ `<Grid item xs={12}>`（MUI v5 语法）
   - ✅ `<Grid size={{ xs: 12, md: 6 }}>`（MUI v7 语法）

2. **月份选择器问题**：
   - 确保使用 `views={['year', 'month']}`
   - 确保安装了 `@mui/x-date-pickers` 和 `date-fns`
   - 确保使用 `LocalizationProvider` 包裹

3. **Autocomplete 组件问题**：
   - 确保 `options` 是数组
   - 确保 `getOptionLabel` 返回字符串
   - 处理空值情况

4. **React Hook Form 错误**：
   - 确保使用 `Controller` 包裹自定义组件
   - 确保 `control` 正确传递
   - 确保表单提交使用 `handleSubmit`

5. **对话框全屏问题**：
   - 使用 `fullScreen={window.innerWidth < 600}`
   - 或使用 MUI 的 `useMediaQuery` Hook

---

## 📚 参考文档

- **设计方案第5节**：`docs/pages/零售合同管理/零售合同管理模块设计方案.md#五前端组件设计`
- **前端页面设计规范**：`docs/前端页面设计规范.md`
- **CLAUDE.md**：前端开发规范
- **参考代码**：
  - `frontend/src/pages/RetailPackagePage.tsx`
  - `frontend/src/components/PackageEditorDialog.tsx`
  - `frontend/src/api/retail-packages.ts`

---

## ✅ 完成后的行动

在完成本会话的所有开发和测试后：

1. 更新开发规划文档中的进度表
2. 清理控制台警告和错误
3. 提交代码到 Git（如果项目使用版本控制）
4. 准备进入会话4（前端详情和完善）

**会话4提示词**：`docs/pages/零售合同管理/会话4-前端详情完善提示词.md`
