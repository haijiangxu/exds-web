...
### 3.8.3. `useTouPeriodBackground` - 图表时段背景色渲染

-   **目的:** 为 Recharts 图表提供标准的“尖、峰、平、谷”等时段背景渲染功能，帮助用户直观地将图表数据与电价时段关联。
-   **文件位置:** `frontend/src/hooks/useTouPeriodBackground.tsx`
-   **核心逻辑:**
    -   自动分析传入的图表数据，根据 `period_type` 字段将连续的时间点合并成一个个时段区间（如 `09:00` 到 `11:00` 为“尖峰”）。
    -   根据时段区间和 `frontend/src/constants/touPeriodColors.ts` 中定义的颜色配置，生成一组可以直接在 Recharts 图表中渲染的 `<ReferenceArea>` 组件。
    -   使用 `useMemo` 进行性能优化，仅在数据源变化时重新计算。
-   **数据格式要求:**
    -   使用此 Hook 的图表，其 `data` 数组中的每个对象**必须**包含以下两个字段：
        -   `time`: `string` - 时间点，格式如 `"00:00"`, `"01:00"`。
        -   `period_type`: `string` - 时段类型，名称需与 `touPeriodColors.ts` 中的 key 匹配，如 `"尖峰"`, `"高峰"`, `"平段"`, `"低谷"`。
-   **使用方法:**
    1.  在图表组件中，从 `useTouPeriodBackground` Hook 中获取 `TouPeriodAreas`。
    2.  将 `TouPeriodAreas` 直接放置在 `<ComposedChart>` 或其他 Recharts 图表组件内部。
